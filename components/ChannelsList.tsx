...
export default async function ChannelsList() {
  ...
  return (
    <div className="py-4 ">
      ...
      <hr className="border border-gray-100 dark:border-neutral-800 rounded my-3" />
            ...
            <div key={cat}>
              ...
              <div className="space-y-2">
                {list.map((ch) => {
                  const canView = canViewLookup[ch._id] || false;
                    return (
                  <div key={ch._id} className="border-2 border-blue-500 dark:border-neutral-900 rounded-lg p-3 flex items-start justify-between bg-neutral-50 dark:bg-black/20 hover:bg-yellow-50 dark:hover:bg-black/40 transition-colors">
                    {canView ? (
                      <Link href={`/?channel=${encodeURIComponent(ch._id)}`} className="flex-1 min-w-0">
                        <div>
                          <div className="font-medium truncate">{ch.name}</div>
